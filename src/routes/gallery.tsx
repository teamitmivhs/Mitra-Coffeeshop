...
function TickerRow({ items, direction = 1, speed = 28 }: {
  items: typeof row1; direction?: 1 | -1; speed?: number;
}) {
  ...
  useEffect(() => {
    ...
    const tick = (ts: number) => {
      if (lastTimeRef.current === null) { lastTimeRef.current = ts; rafRef.current = requestAnimationFrame(tick); return; }
  ...
  return (
    <div
      ...
      onPointerLeave={() => { lastTimeRef.current = null; pausedRef.current = false; }}
    >
...
function TiltCard({ photo, index, onOpen }: {
  photo: typeof photos[0]; index: number; onOpen: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-4 top-4 rounded-full bg-black/30 px-3 py-1 backdrop-blur-sm ring-1 ring-white/15 transition-all duration-300 group-hover:opacity-0 group-hover:-translate-y-1">
        <span className="font-display text-[9px] font-bold uppercase tracking-[0.22em] text-white">{photo.tag}</span>
      </div>
      ...
      >
        <span className="block font-display text-[9px] font-bold uppercase tracking-[0.3em] text-white/55">{photo.tag}</span>
...
function MobileStack({ onOpen }: { onOpen: (i: number) => void }) {
  ...
  const wipeIn  = (dir: 1 | -1) => dir === 1
    ? { clipPath: "polygon(100% 0%, 100% 0%, 100% 100%, 100% 100%)" }
    : { clipPath: "polygon(0% 0%, 0% 0%, 0% 100%, 0% 100%)" };
  const wipeOut = (dir: 1 | -1) => dir === 1
  ...
  return (
    <div className="md:hidden">
      ...
      >
        <AnimatePresence mode="popLayout" custom={direction}>
          ...
          >
            ...
            <div
              ...
              style={{
                background: "linear-gradient(105deg, transparent 48%, rgba(0,0,0,0.18) 48%, rgba(0,0,0,0.18) 52%, transparent 52%)",
              }}
            />
        ...
        <button onClick={() => nav(-1)} className="absolute inset-y-0 left-0 w-1/3" aria-label="Sebelumnya" />
        <button onClick={() => nav( 1)} className="absolute inset-y-0 right-0 w-1/3" aria-label="Berikutnya" />
...
// ─── Lightbox ────────────────────────────────────────────────────────────────
function Lightbox({ photos, index, onClose, onNav, onJump }: {
  photos: typeof photos; index: number;
  onClose: () => void; onNav: (dir: 1 | -1) => void; onJump: (i: number) => void;
}) {
  const [dir, setDir] = useState<1 | -1>(1);

  const go = (d: 1 | -1) => { setDir(d); onNav(d); };
  ...
  const wipeIn  = dir === 1 ? "polygon(100% 0%, 100% 0%, 100% 100%, 100% 100%)" : "polygon(0% 0%, 0% 0%, 0% 100%, 0% 100%)";
  const wipeMid = "polygon(-5% 0%, 105% 0%, 105% 100%, -5% 100%)";
  const wipeOut = dir === 1 ? "polygon(0% 0%, -5% 0%, -5% 100%, 0% 100%)"       : "polygon(100% 0%, 105% 0%, 105% 100%, 100% 100%)";
  ...
  return (
    ...
    >
      <AnimatePresence mode="popLayout" custom={dir}>
        ...
        >
          ...
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-6">
            <p className="font-display text-[10px] font-bold uppercase tracking-[0.28em] text-white/55">{photo.tag}</p>
      ...
      <div className="absolute bottom-5 left-1/2 flex -translate-x-1/2 gap-2">
        {photos.map((_, i) => (
          <button key={i} onClick={(e) => { e.stopPropagation(); onJump(i); }}
        ...
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <motion.h1
              ...
              initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.07 }}
            >
          ...
          <motion.div
            ...
            initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, delay: 0.16 }}
          >
      ...
      <section className="mx-auto max-w-7xl px-4 pb-20 md:hidden">
        <div className="grid grid-cols-3 gap-2">
          {photos.map((photo, i) => (
            ...
            >
              <img src={photo.src} alt={photo.alt} loading="lazy" className="aspect-square w-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
              <figcaption className="absolute inset-x-0 bottom-0 p-2">
                <p className="font-display text-[10px] font-bold leading-tight text-white">{photo.alt}</p>
              </figcaption>
            </motion.figure>
          ))}
        </div>
